...
function newChat(saveIt=true){
 const c={id:uid(),title:"New Chat",created:Date.now(),updated:Date.now(),pinned:false,messages:[]};
 if(state.temporary){state.activeId=c.id;state.tempChat=c}
 else{state.chats.unshift(c);state.activeId=c.id;if(saveIt)save()}
 render();return c
}
...
function renderMessages(){
 ...
 if(!c.messages.length){
  box.innerHTML=`<div class="empty-chat"><div class="empty-icon">✨</div><h2>Hello, Mahlet ✨</h2><p>I'm Global AI Mahlet — your smart assistant.<br>Ask me anything, anytime. I'm here to help you learn, create, solve, and grow! 💙</p></div>`;
  return
 }
...
function renderRecents(filter=""){
 ...
 <div class="recent-chat ${c.id===state.activeId?"active":""}">
...
function render(){
 renderMessages();renderRecents();languageUI();voiceUI();temporaryUI();filePreview()
}
...
async function send(){
 ...
 try{
  ...
  const answer=r?.text||r?.response||r?.message||"I received your message.";
  ...
  typing(false);state.selectedFiles=[];filePreview()
 }
}
...
function typing(show){
 ...
 if(show){
  box.insertAdjacentHTML("beforeend",`<div class="message ai-message typing"><div class="message-bubble">Thinking<span>.</span><span>.</span><span>.</span></div></div>`);
  box.scrollTop=box.scrollHeight
 }
}
...
function addFiles(files){
 if(files)state.selectedFiles.push(...[...files]);
 filePreview()
}
...
function speak(text){
 ...
 u.lang=state.language;u.rate=1;u.pitch=1;
 u.onstart=()=>{};
 u.speechSynthesis;
...
function recognition(){
 ...
 r.onend=()=>{
  ...
  if(state.voiceMode)try{r.start()}catch{}
 };
 r.onerror=e=>{
...
function recentMenu(id){
 ...
 if(a==="1"){
  const n=prompt("New chat name:",c.title);
  if(n?.trim()){c.title=n.trim().slice(0,60);c.updated=Date.now()}
 }
...
function setupActions(){
 document.addEventListener("click",async e=>{
  ...
  if(t.id==="sendButton"||t.matches("[data-send]")||t.classList.contains("send-button")){
   e.preventDefault();send();return
  }

  if(t.id==="micButton"||t.matches("[data-mic]")){
   e.preventDefault();mic();return
  }
  ...
  if(t.matches("[data-share]")){
   ...
   if(!m)return;
   if(navigator.share){
    try{await navigator.share({title:"Global AI Mahlet",text:m.text})}catch{}
   }else if(navigator.clipboard){
    try{
     await navigator.clipboard.writeText(m.text);
     window.showToast?window.showToast("Message copied."):alert("Message copied.")
    }catch{}
   }
   return
  }
...
function setupSearch(){
 const a=[...$$("[data-search-chats]")];
 const r=$("#recentSearch");
 if(r)a.push(r);
 [...new Set(a)].forEach(i=>i.addEventListener("input",()=>renderRecents(i.value)))
}

function setupFiles(){
 $$('input[type="file"]').forEach(i=>i.addEventListener("change",()=>{
  addFiles(i.files);i.value=""
 }));
 $$("[data-camera]").forEach(b=>b.addEventListener("click",e=>{
  e.preventDefault();
  const i=document.querySelector('input[type="file"][capture="environment"]');
  if(i)i.click()
 }));
 $$("[data-attach]").forEach(b=>b.addEventListener("click",e=>{
  e.preventDefault();
  const i=document.querySelector('input[type="file"][data-file-input]');
  if(i)i.click()
 }));
}
