...
function buildVisualQuery(message:string, signals:{distress:boolean;celebration:boolean;encouragement:boolean;imageRequest:boolean}):string{
  ...
  if(signals.imageRequest){
    let query=text
      .replace(/^\s*(show|give|find|send|provide|get)\s+(me\s+)?/i,"")
      ...
      .replace(/^\s*(image|photo|picture|visual|illustration|diagram)\s*[:\-]?\s*/i,"")
      .replace(/[?!.]+$/,"")
      .trim();
    if(!query || /^(it|this|that|something|anything)$/i.test(query))query=text;
...
async function fetchWikimediaVisuals(query:string):Promise<AnswerVisual[]>{
  ...
  try{
    ...
    const response=await fetch(endpoint.href,{headers:{"Accept":"application/json"},redirect:"follow"});
...
async function fetchRelevantVisuals(query:string):Promise<AnswerVisual[]>{
  const wiki=await fetchWikimediaVisuals(query);
  if(wiki.length)return wiki;
  return fetchOpenverseVisuals(query);
}
