...
async function fetchOpenverseVisuals(query:string):Promise<AnswerVisual[]>{
  ...
  try{
    ...
    const terms=cleanQuery.toLowerCase().split(/[^a-z0-9]+/).filter(term=>term.length>=3).slice(0,8);
    const ranked=results.map((item:any,index:number)=>{
      const title=typeof item?.title==="string"?item.title:"";
      const tags=Array.isArray(item?.tags)
        ? item.tags.map((tag:any)=>typeof tag==="string"?tag:String(tag?.name||"")).join(" ")
        : "";
      const searchable=(title+" "+tags).toLowerCase();
