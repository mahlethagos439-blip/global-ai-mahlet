...
function normalizeVisionImage(value: string): string {
  const trimmed = String(value || "").trim();
  if (!trimmed) return "";

  // Cloudflare Workers AI Vision expects the image as a base64 data URL
  // (for example: data:image/jpeg;base64,...). Keep the MIME prefix.
  if (/^data:image\/[^;]+;base64,/i.test(trimmed)) {
    return trimmed;
  }

  // Backward compatibility for older clients that sent raw base64 only.
...
function buildVisualQuery(message:string, signals:{distress:boolean;celebration:boolean;encouragement:boolean;imageRequest:boolean}):string{
  ...
  // For substantive questions, try to provide one relevant real visual even when
  // the topic is not in a predefined category. Do not add random visuals to
  // greetings, very short replies, or simple conversational acknowledgements.
...
Your conversation style should feel as natural, caring, familiar, and attentive as talking with a trusted best friend who genuinely wants to help: listen carefully, remember the immediate context, notice emotional changes, speak naturally, celebrate good moments, stay calm during difficult moments, and help the user find a realistic next step. Keep this warmth conversational rather than theatrical or overly affectionate. ALWAYS be honest that you are an AI. Never claim to be a human, never invent a personal life, and never claim to literally experience emotions. You may express care through your words and recognize the user's emotional situation from their words.
...
`;
      ...
      if (webSearch && !image) {
        ...
        const imageDataUrl = normalizeVisionImage(image);

        // Keep the user's written question together with the image. The
        // browser normally includes the current message in `messages`, but
        // explicitly adding it here also makes the endpoint robust when an
        // older client sends only the previous conversation history.
        const imageMessages = [
          {
            role: "system",
        ...
        const lastMessage = imageMessages[imageMessages.length - 1];
        if (message && (lastMessage?.role !== "user" || lastMessage?.content !== message)) {
          imageMessages.push({ role: "user", content: message });
        }

        /*
         * Cloudflare's Llama 3.2 Vision model accepts the image separately
         * from the messages. IMPORTANT: keep the data:image/...;base64,
         * prefix. Removing it can make multimodal requests fail.
         */
        result = await env.AI.run(
          VISION_MODEL,
          {
