...
function shouldUseWebSearchServer(text:string):boolean{
  ...
  if(!q)return false;
  // Explicit requests to search the real internet.
  if(/\b(search|look up|search the web|search online|search the internet|web search|browse the web|research online|find online|find me|search me|internet)\b/.test(q))return true;
  // Requests for real web resources.
  if(/\b(find|give|show|send|get)\s+(me\s+)?(the\s+)?(official\s+)?(website|link|source|sources|article|articles)\b/.test(q))return true;
  if(/\bofficial\s+(website|page|site)\b/.test(q))return true;
  // Current/fresh questions should use live search.
  ...
  if(/\b(weather|forecast|exchange rate|stock price|live score|live scores|traffic|power outage|news|breaking news)\b/.test(q))return true;
  // Named public facts often benefit from current web evidence when the user
  // explicitly asks for factual lookup language.
  if(/\b(who is|what happened|how much|how many|when is|where is)\b/.test(q) && /\b(2026|2027|2028|today|now|current|latest|official)\b/.test(q))return true;
  return false;
}
...
function buildVisualQuery(message:string, signals:{distress:boolean;celebration:boolean;encouragement:boolean;imageRequest:boolean}):string{
  ...
  // Prefer a concrete educational visual query so image search returns material
  // that teaches the requested concept instead of a generic decorative photo.
  if(/\bphotosynthesis\b/i.test(text))return "photosynthesis diagram plant sunlight carbon dioxide oxygen glucose";
  if(/\bnewton(?:'s)?\s+(?:three\s+)?laws?\b/i.test(text))return "Newton's three laws of motion physics diagram force acceleration action reaction";
  if(/\b(james web[b]?|jwst|webb space telescope)\b/i.test(text))return "James Webb Space Telescope NASA real telescope";
  if(/\b(solar system|planet|planets|space|galaxy|star|moon|sun)\b/i.test(text))return text+" educational astronomy diagram";
  if(/\b(cell|biology|anatomy|human body|chemistry|chemical|physics|electricity|magnet|gravity|atom|molecule|math|geometry|triangle|algebra)\b/i.test(text))return text+" educational diagram labeled";
  if(/\b(animal|bird|flower|plant|tree|ocean|mountain|river|forest|nature|country|city|landmark|museum)\b/i.test(text))return text+" relevant real photo";
...
function visualRelevanceScore(query:string,title:string,description:string,tags:string=""):number{
  const searchable=(title+" "+description+" "+tags).toLowerCase();
  const q=String(query||"").toLowerCase();
  const stop=new Set(["photo","photograph","picture","image","visual","relevant","real","educational","diagram","labeled","the","and","for","with","about","of","in","a","an","to"]);
  const terms=q.split(/[^a-z0-9]+/).filter(t=>t.length>=3&&!stop.has(t));
  if(!terms.length)return 0;
  let score=0;
  for(const term of terms){
    if(searchable.includes(term))score+=3;
  }
  // Strong topic-specific matches matter more than generic words.
  if(/photosynthesis/i.test(q) && /photosynth|chloroplast|sunlight|carbon dioxide|oxygen|glucose|leaf/i.test(searchable))score+=8;
  if(/newton/i.test(q) && /newton|force|motion|acceleration|reaction|inertia/i.test(searchable))score+=8;
  if(/james web|jwst|webb space/i.test(q) && /james webb|jwst|webb space telescope|space telescope/i.test(searchable))score+=10;
  if(/harvard/i.test(q) && /harvard/i.test(searchable))score+=8;
  if(/ethiopia/i.test(q) && /ethiopia|ethiopian/i.test(searchable))score+=8;
  if(/\b(statue|portrait|painting|artwork|logo|seal|flag|shirt|merchandise|book cover|random texture|wallpaper)\b/i.test(searchable))score-=12;
  return score;
}
...
async function fetchWikimediaVisuals(query:string):Promise<AnswerVisual[]>{
  ...
  try{
    ...
    const ranked=pages.map((item,index)=>{
      ...
      const relevance=visualRelevanceScore(query,title,description);
      ...
      return {item,index,score:Math.max(relevance,entityMatches*3)+exactPhrase+physical+bad,placeValid};
    }).filter(x=>x.placeValid && x.score>=4 && (!isPlaceQuery || x.score>0))
...
async function fetchOpenverseVisuals(query:string):Promise<AnswerVisual[]>{
  ...
  try{
    ...
    const ranked=results.map((item:any,index:number)=>{
      ...
      const entityMatches=terms.reduce((n,t)=>n+(searchable.includes(t)?1:0),0);
      const relevance=visualRelevanceScore(query,title,desc,tags);
      const physical=/\b(campus|yard|hall|library|quad|courtyard|gate|tower|entrance|exterior|street|view|aerial|grounds|facade|front|school|university|college)\b/i.test(searchable)?4:0;
      const bad=/\b(statue|sculpture|portrait|painting|artwork|costume|gown|robe|medal|bust|artifact|book cover|logo|seal|flag|shirt|merchandise|player)\b/i.test(searchable)?-10:0;
      return {item,index,score:Math.max(relevance,entityMatches*3)+physical+bad,valid:relevance>=4 && (!isPlaceQuery || (entityMatches>0 && physical>0 && bad===0))};
...
async function runLiveWebSearch(env:Env,systemPrompt:string,message:string){
  ...
  try{
    const primaryResult = await env.AI.run(
      SEARCH_MODEL,
      request,
      { gateway: { id: "default" } }
    );
    const primarySources = extractWebSearchSources(primaryResult);
    if(primarySources.length){
      return { result: primaryResult, directSources:[] as WebSource[] };
    }
    // Some gateway/model responses contain useful text but expose no source
    // annotations. Use the direct live search fallback so the UI can still
    // display actual source cards instead of pretending there were sources.
    try{
      ...
      console.error("Primary search had no source annotations; direct fallback failed:",directError);
      return { result: primaryResult, directSources:[] as WebSource[] };
    }
  }catch(primaryError){
...
- Think about the user's actual goal before composing the answer. Do not merely repeat facts; connect the answer to the question and give the most useful information first.
- Match depth to the question: simple questions get simple answers; difficult questions get a clear explanation with enough detail to be genuinely useful.
- When teaching, explain the idea in plain language first, then give an example, formula, steps, or a deeper explanation when useful.
- When comparing or deciding between options, show the relevant differences clearly and let the user make the final choice.
- When the user asks for a current fact, use live web search when required and distinguish verified information from uncertainty.
- Do not invent missing details. If something is unknown, say what is known and what is still uncertain.
- Use the conversation context so the answer feels continuous rather than like a new generic response.
