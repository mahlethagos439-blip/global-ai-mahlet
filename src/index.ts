...
`;
      ...
      if (webSearch && !image) {
        ...
        /*
         * Cloudflare's Llama 3.2 Vision API supports an image together with
         * the conversation messages.  Keep the data-URL prefix because the
         * official Cloudflare example uses a value such as
        ...
        try {
          ...
          // Some Workers AI deployments/models are more reliable when the
          // current user question is supplied through the prompt form.
          // Retry without discarding the image or the user's text.
          console.error("Vision messages request failed; retrying with prompt:", visionError);

          const currentQuestion = message ||
            "Please analyze and understand the image I provided.";
