...
function extractVisualSubject(message:string):string{
  const text=String(message||"").replace(/\s+/g," ").trim();
  if(!text)return "";
  const explicit=text.match(/\b(?:real|actual|genuine|true|relevant)?\s*(?:photos?|photographs?|images?|pictures?|visuals?)\s+of\s+(.+?)(?:\s+(?:please|right now))?[?!.]*$/i);
  if(explicit?.[1] && !/^its\s+/i.test(explicit[1]))return explicit[1].trim().replace(/^the\s+/i,"");
  const possessive=text.match(/\b(?:photos?|photographs?|images?|pictures?|visuals?)\s+of\s+its\s+[^?!.]+[?!.]*$/i);
  if(possessive){
    const before=text.slice(0, possessive.index ?? 0);
    const subject=before.match(/\b(?:what|who|where)\s+is\s+(?:the\s+)?([^?!.]+)[?!.]/i);
    if(subject?.[1])return subject[1].trim();
  }
  const questionSubject=text.match(/\b(?:what|who|where)\s+is\s+(?:the\s+)?([^?!.]+)[?!.]/i);
  if(questionSubject?.[1])return questionSubject[1].trim();
  const about=text.match(/\b(?:about|regarding)\s+([^?!.]+?)(?:\s+and\s+|[?!.]|$)/i);
...
function buildVisualQuery(message:string, signals:{distress:boolean;celebration:boolean;encouragement:boolean;imageRequest:boolean}):string{
  ...
  if(signals.imageRequest){
    let query=extractVisualSubject(text);
    if(!query){
      query=text.replace(/\b(please|could you|can you|would you|i want you to|i need you to|for me|relevant|real|actual|genuine|true)\b/gi," ")
        .replace(/\b(show|give|find|send|provide|get|fetch|display|search)\b/gi," ")
        .replace(/\b(?:images?|photos?|photographs?|pictures?|visuals?|illustrations?|diagrams?)\b/gi," ")
        .replace(/\s+/g," ").trim();
    }
    if(!query || /^(it|this|that|something|anything|me|its|its campus)$/i.test(query))query=text;
    if(/\b(university|college|campus|school|museum|airport|hospital|stadium|library|church|mosque|cathedral|monument|landmark|building|palace|bridge|tower)\b/i.test(query)){
      const named=query.replace(/\b(actual|real|genuine|true|photo|photograph|picture|image|visual|illustration|relevant)\b/gi," ").replace(/\s+/g," ").trim();
      if(named){
        if(/\bharvard\b/i.test(named))query="Harvard University campus Harvard Yard Massachusetts real photograph";
        else if(/\bmit\b|massachusetts institute of technology/i.test(named))query="MIT campus Massachusetts Institute of Technology Cambridge real photograph";
        else if(/\bduke\b/i.test(named))query="Duke University campus Durham North Carolina real photograph";
        else if(/\brice\b/i.test(named))query="Rice University campus Houston Texas real photograph";
        else if(/\bcolumbia\b/i.test(named))query="Columbia University campus New York City real photograph";
        else query=`${named} exterior campus building real photograph`;
      }
    }
  ...
  if(text.length>=18 && !/^(hi|hello|hey|thanks|thank you|ok|okay|yes|no|sure|good morning|good evening|how are you)[.!?\s]*$/i.test(text))return text.slice(0,180)+" relevant educational or contextual photo illustration";
  return "";
}
...
function isPhysicalPlaceVisual(query:string, title:string, description:string):boolean{
  ...
  const qTerms=q.split(/[^a-z0-9]+/).filter(t=>t.length>=3 && !/^(photo|photograph|picture|image|visual|real|actual|genuine|true|relevant|campus|building|university|college|school|the|of|in|and|yard|city|massachusetts|north|carolina|texas)$/i.test(t));
...
async function fetchWikimediaVisuals(query:string):Promise<AnswerVisual[]>{
  ...
  try{
    ...
    const terms=query.toLowerCase().split(/[^a-z0-9]+/).filter(t=>t.length>=3 && !/^(photo|photograph|picture|image|visual|real|actual|genuine|true|relevant|campus|building|university|college|school|the|of|in|and|yard|city|massachusetts|north|carolina|texas)$/i.test(t));
  ...
  async fetch(
    ...
    try {
      ...
      if (url.pathname === "/api/title") {
        ...
        const titleHistory=Array.isArray(body.messages)
          ? body.messages.slice(-10).map((m:any)=>`${m?.role||"user"}: ${String(m?.content||m?.text||"").slice(0,800)}`).join("\n")
          : "";
        const titleInput=[titleHistory, titleMessage ? `Latest user message: ${titleMessage}` : ""].filter(Boolean).join("\n");
        if(!titleInput){
      ...
      const emotionalSignals = {
        ...
        imageRequest: /\b(?:show|give|find|send|provide|get|fetch|display)\b[\s\S]{0,100}\b(?:image|images|photo|photos|photograph|photographs|picture|pictures|visual|visuals|illustration|illustrations|diagram|diagrams)\b/i.test(message) || /\b(?:real|actual|genuine|true|relevant)\s+(?:image|images|photo|photos|photograph|photographs|picture|pictures|visual|visuals)\b/i.test(message) || /\b(?:image|images|photo|photos|photograph|photographs|picture|pictures|visual|visuals)\s+of\b/i.test(message),
      };
...
${emotionalSignals.imageRequest ? "The user is requesting a visual. Answer the user's question normally AND let the UI display relevant returned visuals underneath. Never replace the written answer with a generic sentence about photos. Never invent image URLs or claim an image was found unless the returned visual data actually contains one." : ""}

IMAGE UNDERSTANDING: If an image is attached, treat it as real input. Carefully inspect visible objects, people, scenes, diagrams, layout, and readable text. Answer the user's exact question using the image. If the user says “understand this photo” without a specific question, give a useful description of what the image contains and explain the most important context you can infer from visible evidence. Do not say you cannot access, see, or analyze the image when an image was supplied to the Vision model. Do not invent details that are not visible.

VISUAL ANSWERS: When a relevant visual is returned by the application, it is supplementary to your written answer. Your written answer must still answer the user's question. For explicit real-photo requests, the application will search for the exact requested subject; do not substitute an associated object, logo, currency, map, animal, artwork, or merchandise for the requested subject.
NEVER write image URLs, markdown image syntax, [Image: ...] placeholders, or invented photo links in the answer. The application supplies real image cards separately.
`;
      ...
      if (webSearch && !image) {
        ...
        try {
          result = await env.AI.run(
            ...
            {
              messages: [
                { role: "system", content: systemPrompt + "\n\nIMPORTANT: An image is attached. Analyze it directly. Never claim you cannot see or analyze it. Read visible text when possible, answer the user's exact request, and distinguish visible facts from uncertainty." },
          ...
          const qwenText=String(result?.response||result?.result?.response||result?.choices?.[0]?.message?.content||result?.output_text||result?.result?.output_text||result?.description||"");
          if(!qwenText.trim() || /(don't|do not|cannot|can't) (see|access|analy[sz]e).*image|text[- ]only|no photo attached|unable to (see|analy[sz]e)/i.test(qwenText))throw new Error("Qwen Vision returned an unusable image response.");
        } catch (qwenError) {
          console.error("Primary Qwen Vision request failed; trying Llama Vision:", qwenError);
          try {
            ...
            result = await env.AI.run(
              ...
              {
                messages: [
                  { role: "system", content: systemPrompt + "\n\nIMPORTANT: An image is attached. Analyze it directly. Never claim you cannot see or analyze it." },
                  { role: "user", content: message || "Please analyze and understand the image I provided." },
                ],
                image: imageBase64,
                max_tokens: 1400,
            ...
            const llamaText=String(result?.response||result?.result?.response||result?.choices?.[0]?.message?.content||result?.output_text||result?.result?.output_text||result?.description||"");
            if(!llamaText.trim() || /(don't|do not|cannot|can't) (see|access|analy[sz]e).*image|text[- ]only|no photo attached|unable to (see|analy[sz]e)/i.test(llamaText))throw new Error("Llama Vision returned an unusable image response.");
          } catch (fallbackError) {
            console.error("Fallback Llama Vision request failed:", fallbackError);
            const primaryDetail=qwenError instanceof Error?qwenError.message:String(qwenError||"Unknown Qwen Vision error");
            const fallbackDetail=fallbackError instanceof Error?fallbackError.message:String(fallbackError||"Unknown Llama Vision error");
            throw new Error(`Image analysis failed. Primary Vision error: ${primaryDetail}. Fallback Vision error: ${fallbackDetail}`);
          }
        }
      ...
      // Keep the AI's written answer. Visuals are supplemental cards, never a replacement.
      // For explicit image requests we always search for the requested subject.
      // For ordinary substantive questions, visualQuery may add a relevant visual
      // when the topic is naturally visual (science, places, animals, etc.).
      const shouldShowVisuals = Boolean(visualQuery) && (
        emotionalSignals.imageRequest ||
        /\b(solar system|planet|space|galaxy|animal|bird|flower|plant|tree|ocean|mountain|river|forest|city|country|landmark|museum|university|college|campus|school|physics|chemistry|biology|anatomy|photosynthesis|atom|molecule|geometry|triangle|diagram)\b/i.test(message)
      );
      const visuals=shouldShowVisuals ? await fetchRelevantVisuals(visualQuery) : [];

      let finalAnswer = webSearch && !image
        ? cleanWebSearchAnswer(String(answer))
        : String(answer);

      // Never replace the AI answer with a generic image sentence. The UI can
      // render the visuals underneath the answer, just like a multimodal chat.
      if(emotionalSignals.imageRequest && !visuals.length){
        finalAnswer += `\n\nI couldn't find a suitable real image for “${visualQuery || message}” right now.`;
      }
