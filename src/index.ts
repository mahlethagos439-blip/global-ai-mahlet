...
function buildVisualQuery(message:string, signals:{distress:boolean;celebration:boolean;encouragement:boolean;imageRequest:boolean}):string{
  ...
  if(!text)return "";
  if(/^(hi|hello|hey|good morning|good afternoon|good evening|how are you|how r you|thanks|thank you|ok|okay)[!.? ]*$/i.test(text))return "";
  ...
  if(signals.distress)return text.slice(0,180)+" supportive emotional wellbeing conversation";
  if(signals.celebration)return text.slice(0,180)+" celebration achievement success";
  if(signals.encouragement)return text.slice(0,180)+" motivation goal achievement";
  ...
  if(/\b(animal|bird|flower|plant|tree|ocean|mountain|river|forest|nature|country|city|landmark|museum|africa|ethiopia|addis ababa)\b/i.test(text))return text+" photo";
  if(/\b(code|coding|programming|software|computer|robot|artificial intelligence|\bAI\b|technology)\b/i.test(text))return text+" technology";
  // For substantive questions, search for a real visual directly related to the user's topic.
  if(text.length>=12 || /\?$/.test(text))return text.slice(0,180)+" relevant photo illustration";
  return "";
}

async function fetchOpenverseVisuals(query:string, excludeUrls:string[]=[]):Promise<AnswerVisual[]>{
  if(!query)return [];
  try{
    ...
    const seen=new Set<string>(excludeUrls.filter(Boolean));
...
export default {
  async fetch(
    ...
    try {
      ...
      const excludeVisualUrls = Array.isArray(body.excludeVisualUrls)
        ? body.excludeVisualUrls.filter((v:any)=>typeof v === "string").slice(-30)
        : [];
      ...
      const emotionalSignals = {
        distress: /\b(sad|depressed|hopeless|cry|crying|failed|failure|lonely|scared|afraid|worried|anxious|stressed|stress|hurt|broken|tired|give up|can\'t do|cannot do|lost|overwhelmed|bad day|bad situation|difficult situation|terrible situation|not okay|not ok|falling apart|problem|personal life|struggling|pain)\b/i.test(message),
...
- Do not add a speaker label such as “Global AI Mahlet · Neutral” or “Global AI Mahlet · Thinking” before the answer. The application handles the interface around your response.
- When a relevant real visual is supplied by the application, answer normally and let the visual complement the text; never claim an image was found unless the application actually supplied one.
...
`;
      ...
      const visuals=visualQuery && !image
        ? await fetchOpenverseVisuals(visualQuery, excludeVisualUrls)
        : [];
      ...
      return json(
        {
          ...
          responseStyle: {
            ...
            visualRequested: Boolean(visualQuery),
