...
`;
      ...
      if (webSearch && !image) {
        ...
        try {
          // PRIMARY: Qwen 3.8 27B is explicitly documented by Cloudflare as
          // an Image-Text-to-Text model. It accepts both `image` (base64) and
          // `messages`, so the photo and the user's question stay connected.
          // IMPORTANT: For Cloudflare's multimodal chat format, the image
          // must be part of the USER message content. Sending the image as a
          // separate top-level field can result in the model receiving only
          // the text and replying that no photo was attached.
          result = await env.AI.run(
            VISION_MODEL,
            {
              messages: [
                ...
                },
                {
                  ...
                  content: [
                    {
                      type: "text",
                      text: message || "Please analyze and understand the image I provided.",
                    },
                    {
                      type: "image_url",
                      image_url: {
          ...
          // Some multimodal models can return a text-only refusal instead of
          // throwing an exception. Treat that as a failed vision response and
          // use the real vision fallback rather than showing a false answer.
          ...
          const refusedVision =
            ...
            /text (only|you) can provide/.test(visionText);

          if (refusedVision) {
            throw new Error("Primary vision model returned a text-only refusal.");
          }
        } catch (visionError) {
          console.error("Primary Vision request failed; trying Llama fallback:", visionError);

          try {
            // FALLBACK: Cloudflare's documented Llama 3.2 Vision model. The
            // first call accepts Meta's license agreement; later calls use the
            // normal multimodal image + messages request.
            if (!visionLicenseAgreed) {
              await env.AI.run(VISION_FALLBACK_MODEL, { prompt: "agree" });
