...
async function fetchRelevantVisuals(query:string):Promise<AnswerVisual[]>{
  ...
  for(const q of variants){
    ...
    for(const item of wiki){const key=item.image||item.thumbnail||item.url||"";if(!key||seen.has(key))continue;seen.add(key);collected.push(item);if(collected.length>=3)return collected;}
  }
  for(const q of variants){
    const openverse=await fetchOpenverseVisuals(q);
    for(const item of openverse){const key=item.image||item.thumbnail||item.url||"";if(!key||seen.has(key))continue;seen.add(key);collected.push(item);if(collected.length>=3)return collected;}
  }
  return collected;
}
