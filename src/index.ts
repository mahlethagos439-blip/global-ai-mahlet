...
function buildVisualQuery(message:string, signals:{distress:boolean;celebration:boolean;encouragement:boolean;imageRequest:boolean}):string{
  ...
  if(signals.imageRequest){
    let query=text
      .replace(/\b(please|could you|can you|would you|i want you to|i need you to)\b/gi," ")
      .replace(/^\s*(show|give|find|send|provide|get|fetch|display)\s+(me\s+)?/i,"")
      .replace(/^\s*(the\s+)?(?:an?\s+)?(actual|real|genuine|true)\s+(image|photo|picture|visual|illustration|diagram)\s+(of|about)\s+/i,"")
      .replace(/^\s*(an?|the)\s+(image|photo|picture|visual|illustration|diagram)\s+(of|about)\s+/i,"")
      .replace(/^\s*(image|photo|picture|visual|illustration|diagram)\s*(of|about)?\s*/i,"")
      .replace(/^\s*(of|about)\s+/i,"")
      .replace(/[?!.]+$/," ")
      .replace(/\s+/g," ")
      .trim();

    // If the request still contains conversational filler, prefer the
    // subject after "of/about". Example: "please give me the actual photo
    // of lion" -> "lion" rather than the whole sentence.
    const ofMatch=query.match(/\b(?:of|about)\s+(.+)$/i);
    if(ofMatch)query=ofMatch[1].trim();

    // Remove trailing request words that are not part of the subject.
    query=query
      .replace(/\b(right now|please|for me|to me)\b/gi," ")
      .replace(/\s+/g," ")
      .trim();

    // Common singular/plural request subjects should remain intact.
    if(!query || /^(it|this|that|something|anything|me)$/i.test(query)){
      const fallback=text.match(/\b(?:of|about)\s+([a-z][a-z0-9 -]{1,100})/i);
      query=fallback?fallback[1].trim():text;
    }
    query=query.replace(/[?!.]+$/g,"").trim();

    // For named universities/colleges, search for the physical campus rather
    // than generic objects associated with the institution (for example,
    // Harvard regalia or artwork at a Harvard museum). The user asked for a
    // real photo of the place itself.
    if(/\b(university|college|campus|school)\b/i.test(query)){
      const named=query
        .replace(/\b(actual|real|genuine|true)\b/gi," ")
        .replace(/\b(photo|photograph|picture|image|visual|illustration)\b/gi," ")
        .replace(/\s+/g," ").trim();
      if(named){
        if(/\bharvard\b/i.test(named)) query="Harvard University campus Harvard Yard";
        else if(/\bmit\b|massachusetts institute of technology/i.test(named)) query="MIT campus Massachusetts Institute of Technology";
        else if(/\bduke\b/i.test(named)) query="Duke University campus";
        else if(/\brice\b/i.test(named)) query="Rice University campus";
        else if(/\bcolumbia\b/i.test(named)) query="Columbia University campus New York";
        else query=`${named} campus building`;
      }
    }
...
async function fetchWikimediaVisuals(query:string):Promise<AnswerVisual[]>{
  ...
  try{
    ...
    endpoint.searchParams.set("gsrsearch",query.replace(/\s+/g," ").trim().slice(0,120));
    endpoint.searchParams.set("gsrnamespace","6");
    endpoint.searchParams.set("gsrlimit","12");
    endpoint.searchParams.set("prop","imageinfo");
    endpoint.searchParams.set("iiprop","url|extmetadata");
    endpoint.searchParams.set("iiurlwidth","1000");
    endpoint.searchParams.set("format","json");
    endpoint.searchParams.set("origin","*");

    const response=await fetch(endpoint.href,{headers:{"Accept":"application/json","User-Agent":"GlobalAIMahlet/1.0"},redirect:"follow"});
    if(!response.ok)return [];
    const data:any=await response.json();
    const pages=Object.values(data?.query?.pages||{}) as any[];
    const terms=query.toLowerCase().split(/[^a-z0-9]+/).filter(t=>t.length>=3);
    const isPlaceQuery=/\b(university|college|campus|school)\b/i.test(query);
    const placeTerms=terms.filter(term=>!/(photo|photograph|campus|building|university|college|school|in|the|of|and)/i.test(term));
    const ranked=pages.map((item,index)=>{
      ...
      const entityMatches=placeTerms.reduce((sum,term)=>sum+(searchable.includes(term)?1:0),0);
      const campusBonus=/\b(campus|yard|hall|building|library|quad|courtyard|gate|tower|school|university)\b/i.test(searchable)?2:0;
      return {item,index,score,entityMatches,campusBonus};
    })
      .filter(x=>x.score>0 && (!isPlaceQuery || x.entityMatches>0))
      .sort((a,b)=>(b.entityMatches-a.entityMatches)||(b.campusBonus-a.campusBonus)||(b.score-a.score)||a.index-b.index);
    ...
    for(const row of ranked){
      ...
      const clean=(v:any)=>String(v?.value||"").replace(/<[^>]+>/g," ").replace(/&nbsp;/g," ").replace(/\s+/g," ").trim();
      const title=String(item?.title||"Related photo").replace(/^File:/i,"").replace(/\.[a-z0-9]+$/i,"").trim();
      const page=typeof info?.descriptionurl==="string"?info.descriptionurl:"";
      visuals.push({
        image,
        thumbnail:image,
        title:title||"Related photo",
        url:page,
        creator:clean(meta?.Artist),
        license:clean(meta?.LicenseShortName),
    ...
    console.warn("Wikimedia visual search failed:",error);
    return [];
  }
}
...
async function fetchOpenverseVisuals(query:string):Promise<AnswerVisual[]>{
  ...
  try{
    ...
    const response=await fetch(endpoint.href,{method:"GET",redirect:"follow",headers:{"Accept":"application/json","User-Agent":"GlobalAIMahlet/1.0"}});
    if(!response.ok)return [];
    const data:any=await response.json();
    const results=Array.isArray(data?.results)?data.results:[];
    const terms=cleanQuery.toLowerCase().split(/[^a-z0-9]+/).filter(term=>term.length>=3);
    ...
    }).filter((x:any)=>x.score>0).sort((a:any,b:any)=>b.score-a.score||a.index-b.index);
...
async function fetchRelevantVisuals(query:string):Promise<AnswerVisual[]>{
  ...
  const variants=/\b(university|college|campus|school)\b/i.test(base)
    ? [base, `${base} photo`, `${base} campus photo`, `${base} building photo`]
    : [base, `${base} photo`, `${base} photograph`];
  const seen=new Set<string>();
  const collected:AnswerVisual[]=[];
