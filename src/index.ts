...
async function verifyWebSources(sources:WebSource[]):Promise<WebSource[]>{
  const checked=await Promise.all(sources.slice(0,8).map(async source=>{
    if(!source.url)return null;
    try{
      const controller=new AbortController();
      const timer=setTimeout(()=>controller.abort(),5000);
      const response=await fetch(source.url,{method:"GET",headers:{"User-Agent":"Global-AI-Mahlet/1.0","Accept":"text/html,application/xhtml+xml"},signal:controller.signal});
      clearTimeout(timer);
      if(!response.ok)return null;
      return source;
    }catch{return null;}
  }));
  return checked.filter(Boolean) as WebSource[];
}
            ...
            messages:[
              {
                role:"system",
                content:
                  `Create a short title for this conversation. Return ONLY the title, with no quotation marks, no punctuation at the end, and no explanation. Use 2 to 6 words. The title should describe the user's intent, not copy their exact sentence. If the user only greets you (for example hi, hello, hey, hii), use a natural title such as Greeting or Hello. The user's language is ${titleLanguage}.`
              },
...
`;
      ...
      if (webSearch && !image) {
            ...
            input: [
              {
                ...
                content:
                  systemPrompt +
                  "\nUse the live web-search tool before answering. For current or time-sensitive claims, rely on retrieved results rather than model memory. Every source you mention must come from the tool results. If reliable results are unavailable, say you could not verify the information. Do not invent a source, date, URL, quote, or event.",
              },
              {
                role: "user",
                content: message,
        ...
        sources = await verifyWebSources(sources);
      } else if (image) {
        recordEvent(
          env,
          "image_analysis",
