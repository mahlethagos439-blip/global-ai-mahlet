...
const VISION_FALLBACK_MODEL = "@cf/google/gemma-4-26b-a4b-it";

// Cloudflare requires a one-time Meta license/Acceptable Use agreement
// before the Llama 3.2 Vision model can be used. Keep this per Worker
// isolate so normal image chats do not repeat the agreement unnecessarily.
...
`;
      ...
      if (webSearch && !image) {
        ...
        try {
          // Cloudflare requires the first Llama 3.2 Vision request to accept
          // Meta's License and Acceptable Use Policy. This agreement is
          // account-level, so once it succeeds we can use the model normally.
          if (!visionLicenseAgreed) {
            await env.AI.run(VISION_MODEL, { prompt: "agree" });
            visionLicenseAgreed = true;
          }

          // Use the documented multimodal shape: conversation messages plus
          // the separate image field. This is more reliable than putting the
          // whole conversation into one prompt string and keeps the user
          // message and photo explicitly connected.
          result = await env.AI.run(
            VISION_MODEL,
            {
              messages: [
          ...
          // Keep the existing Llama path first, but do not let a provider-side
          // Vision failure make photo + text completely unusable. Cloudflare
          // currently also provides Gemma 4 with Vision, and it does not use
          // the Meta Llama agreement. Try it as a transparent fallback.
          console.error("Primary Vision request failed; trying fallback:", visionError);
