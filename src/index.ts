...
async function fetchRelevantVisuals(query:string):Promise<AnswerVisual[]>{
  ...
  // Search the exact user intent first, then a small number of targeted
  // variants. This keeps the results relevant without flooding the UI.
  const variants=place
    ? [base, `${base} exterior`, `${base} campus`, `${base} building`, `${base} grounds`]
    : [base, `${base} photo`, `${base} photograph`, `${base} real photo`];
  ...
  async fetch(
    ...
    try {
      ...
      const emotionalSignals = {
        ...
        // IMPORTANT: ordinary questions must NOT automatically receive images.
        // Image search runs only when the user explicitly asks for an
        // image/photo/picture/visual, or clearly asks what something looks like.
        imageRequest:
          /\b(real|actual|genuine|existing|original)\s+(image|photo|photograph|picture|pictures|photos)\b/i.test(message) ||
          /\b(image|images|photo|photos|photograph|photographs|picture|pictures|visuals|illustration|illustrations|diagram|diagrams)\b/i.test(message) ||
          /\b(show|give|send|find|get|provide|display)\s+(me\s+)?(some\s+|a\s+|the\s+)?(real\s+|actual\s+|relevant\s+)?(image|images|photo|photos|photograph|photographs|picture|pictures)\b/i.test(message) ||
          /\b(what\s+does|how\s+does)\b[^?]{0,120}\b(look\s+like|looks\s+like)\b/i.test(message),
      };
...
VISUALS:
- If the user explicitly asks for a real image/photo/picture/visual, treat it as a real web-image-search request.
- The application searches real public web-image indexes (currently Wikimedia Commons and Openverse) and displays the returned image URLs. Do not invent, fabricate, or describe a generated image as a real photograph.
...
- When relevant visuals are returned, briefly introduce them naturally (for example, "Here are some relevant real photos.") and keep the written answer focused on the user's request.
- Do not attach unrelated images to ordinary questions merely to make an answer look attractive.
- If no reliable matching real image is found, say that no suitable real image was found rather than showing an unrelated image.
...
`;
      ...
      // Never attach decorative/random images to ordinary answers.
      // Only run the real-image search when the user explicitly requested
      // images/photos/pictures/visuals.
      const visuals =
        emotionalSignals.imageRequest && visualQuery && !image
