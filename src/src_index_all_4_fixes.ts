  ...
  async fetch(
    ...
    try {
      ...
      const emotionalSignals = {
        ...
        imageRequest: /\b(show|give|find|send|provide|get|fetch|display)\s+(?:me\s+)?(?:the\s+)?(?:a\s+|an\s+)?(?:real\s+|actual\s+)?(?:image|photo|photograph|picture|visual|illustration|diagram)\b/i.test(message) || /\b(?:real|actual)\s+(?:image|photo|photograph|picture)\b/i.test(message) || /\b(?:image|photo|photograph|picture)\s+of\b/i.test(message),
      };
