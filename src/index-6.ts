...
`;
      ...
      if (webSearch && !image) {
        ...
        try {
          // IMPORTANT: Cloudflare requires the initial Vision-model request
          // to accept Meta's License and Acceptable Use Policy. Without this,
          // an otherwise-correct photo request can fail before the model sees
          // the image. We do it automatically once per Worker isolate.
          if (!visionLicenseAgreed) {
            await env.AI.run(VISION_MODEL, {
              prompt: "agree",
            });
          ...
          const detail = visionError instanceof Error
            ? visionError.message
            : String(visionError || "Unknown Vision AI error");
          throw new Error(
