...
function isPhysicalPlaceVisual(query:string, title:string, description:string):boolean{
  ...
  const badHit=/\b(statue|sculpture|portrait|painting|artwork|costume|gown|robe|medal|bust|museum object|artifact|book cover|logo|seal|flag|shirt|merchandise|football player|basketball player|map|floor plan|site plan|diagram|drawing|blueprint|postcard)\b/i.test(searchable);
...
async function fetchWikimediaVisuals(query:string):Promise<AnswerVisual[]>{
  ...
  try{
    ...
    const ranked=pages.map((item,index)=>{
      ...
      const bad=/\b(statue|sculpture|portrait|painting|artwork|costume|gown|robe|medal|bust|artifact|book cover|logo|seal|flag|shirt|merchandise|player|map|floor plan|site plan|diagram|drawing|blueprint|postcard)\b/i.test(searchable)?-10:0;
...
async function fetchRelevantVisuals(query:string):Promise<AnswerVisual[]>{
  ...
  // Use more than one real image provider instead of permanently preferring
  // one provider. The first provider supplies the strongest matches, while
  // the second provider is also queried so the response can contain genuinely
  // different source domains when good matches exist.
  const seen=new Set<string>();
  const collected:AnswerVisual[]=[];

  const addUnique=(items:AnswerVisual[], limit:number)=>{
  ...
  // Gather up to two strong Wikimedia matches.
  ...
  // Always give the second provider a chance. This prevents the source icon
  // from becoming permanently tied to Wikimedia when Openverse has a useful
  // independent result.
  for(const q of variants){
    const openverse=await fetchOpenverseVisuals(q);
    addUnique(openverse,3);
    if(collected.length>=3)break;
  }

  // If one provider could not supply enough results, fill the remaining slots
  // from the other provider rather than returning unrelated images.
...
function cleanVisualAnswer(value:string):string{
  let text=String(value||"").replace(/\r/g,"").trim();

  // Image-search results are rendered as visual cards by the application.
  // Never expose raw image URLs, markdown links, or source-list markup inside
  // the assistant's written response.
  text=text.replace(/\[[^\]\n]{1,220}\]\(\s*https?:\/\/[^)]+\s*\)/gi,"");
  text=text.replace(/^\s*\[[^\]\n]{1,220}\]\s*$/gim,"");
  text=text.replace(/^\s*\(?https?:\/\/\S+\)?\s*$/gim,"");
  text=text.replace(/^\s*(?:source|sources|reference|references)\s*[:\-].*$/gim,"");

  // Keep the written response concise. The visual cards already contain the
  // individual image titles/captions, so the model should not repeat them.
...
function compactVisualSources(visuals:AnswerVisual[]):WebSource[]{
  const result:WebSource[]=[];
  const seenHosts=new Set<string>();
  for(const visual of visuals){
    if(!visual?.url)continue;
    try{
      const parsed=new URL(String(visual.url));
      const host=parsed.hostname.toLowerCase().replace(/^www\./,"");
      if(!host || seenHosts.has(host))continue;
      seenHosts.add(host);
      result.push({title: visual.provider || host,url: String(visual.url)});
...
- For explicit real-photo requests, keep the written response to 1–2 short paragraphs. Do not list individual image titles, image URLs, source names, or repeated descriptions of each photo; those are rendered by the UI as image cards and source icons.
...
`;
      ...
      if(!image && visuals.length){
        // Real-photo searches are externally sourced, but the UI should show
        // only a small number of actual source-domain icons rather than one
        // icon for every individual image.
        const visualSources=compactVisualSources(visuals);
      ...
      // Explicit image requests get a clean written response plus the actual
      // image cards. Do not expose raw URLs or repeat every image's metadata.
      if(!image && emotionalSignals.imageRequest){
        finalAnswer=cleanVisualAnswer(finalAnswer);
        if(visuals.length){
          finalAnswer = `${finalAnswer ? finalAnswer + "\n\n" : ""}Here are relevant real photos of what you asked for.`.trim();
        }else{
